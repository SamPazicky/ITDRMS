...
* `data` - a data frame with clean data ready for further processing
...
ITDRMS has two different functions to identify hits. Both of them identify hits based on the overlap of confidence intervals between control temperature and thermal each thermal challenge temperature at each dose. The calculated values resemble p-values.

In ITDRMS.hit0, these values are adjusted and weighted by R2 goodness-of-fit. During R2-weighting, the scores will slightly increase if they are above the `R2line` and decrease if they are below the `R2line`. The resulting score is **Confidence index** and is considered significant above -log10(0.05). The x-axis corresponds to differences in the areas under the curve (dAUC) and the nMAD argument sets the dAUC cutoff.

```{r ITDR_hits0}
ITDR_hits <- ITDRMS.hit0(ITDR_fitted$data, nMAD=1)
```
...
* `R2line` - double: Soft R2 cut-off. Proteins with R-squared value below this value will not be considered as hits despite favourable dAUC and CI.
* `plot.settings` - list of graphical settings for plot. Defaults are: 

```{r ITDR_hits0_plotsettings}
list(labels=TRUE, 
     label.text.size=2.5, 
     label.force=1.3, 
...
* `data` - a data frame with dAUC and confidence indeces for all proteins
* `plot` - the volcano plot
* `hitlist` - a list with two vectors: one with stabilized and one with destabilized hits.

The second function, ITDRMS.hit, does not adjust the confidence indeces based on R2 values and uses total response on x-axis. Total response is a sum of max response (maximum response at any of the thermal challenge temperatures) and sum of all responses at each thermal challenge temperature.

```{r ITDR_hits}
ITDR_hits <- ITDRMS.hit(ITDR_fitted$data)
```

The function has the following attributes:
* `data` - data frame: Scaled data with removed outliers and fitting statistics, ideally $data element from ITDRMS.fit output.
* `CIthreshold` - a numeric vector of length 2: the confidence index cutoff values for hits (first value) and candidates (second value).
* `minresponse` - a numeric vector of length 2: the total response cutoff for hits (first value) and  candidates (second value).
* `R2line` - a numeric vector of length 2: Soft R2 cut-off. Proteins with R-squared value below this value will not be considered as hits despite favourable total response and confidence index. Cutoff for hits (first value) and candidates (second value).
* `POI` - a character vector: IDs of proteins of interest.
* `plot.settings` - list of graphical settings for plot.

The available plot settings are same as for ITDRMS.hit0.

The function returns three elements:
* `data` - a data frame with responses and confidence indeces for all proteins
* `plot` - the volcano plot
* `hitlist` - a list with two vectors: one with stabilized and one with destabilized hits.
...
* `print.stats` - logical: Should the R2 and confidence indeces be printed in the plots? Default is FALSE.
